...
var recommendation = {
    ...
    products: function(){
        for (var i=0; i < 10; i++){
            $('.col-1').append('<li class="product" data-id="'+i+'">'+products_svg+'<span class="name"></span></li>');
        }
        for (var i=10; i < 20; i++){
            $('.col-2').append('<li class="product" data-id="'+i+'">'+products_svg+'<span class="name"></span></li>');
        }
    },
    submit: function(){
        var user = $("#users").val();
        ...
        $.ajax({
            ...
            dataType : "application/json", // the type of the data sent FROM the server
            contentType: "application/json", // the type of the data to be sent to the server
            success: function(d) { 
                // the data you will receive from the server
                // the type is the json formatted string, wrapped around in the Reponse object in Flask
                // but it will be in the formate of the applicaiton/json type

                recommendation.reset();
                products = JSON.parse(d)['products'];
                $("#result").show().children('code').text(products);
                if (products.length >= 0){
                    $('.results p').addClass('show');
                    this.products();
                }
                for (var i=0; i < (products.length/2); i++){
                    var $this_product = $('.col-1 .product:nth-child('+(i+1)+')');
                    $this_product.find('.name').html(products[i]);
                    $this_product.addClass('show');
                }
                for (var i=(products.length/2); i < products.length; i++){
                    var $this_product = $('.col-2 .product:nth-child('+(i+1)+')');
                    $this_product.find('.name').html(products[i]);
                    $this_product.addClass('show');
                }
                Prism.highlightAll();
